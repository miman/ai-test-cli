...
function fn(target: string, entityType: string, organizationId: string): string {
  ...
  if (entityType.includes('vehicle')) {
    ...
    return `The entity closest to [${target}] is '${JSON.stringify(ent[0], null, 2)}'`
  } else {
...
function toolDef(): Tool {
  const tool: Tool = {
    ...
    function: {
      name: 'getClosest',
      description: 'returns the entity closest to a place given the locations of a list of entities and the place',
      parameters: {
        type: 'object',
        properties: {
          target: {
          ...
          entityType: {
            type: 'string',
            description: 'The type of entity that you want to find the closest one of',
          },
          organizationId: {
            type: 'string',
